...
def test_single_bank():
    game = Game()
    errors = diff(game.play, path="tests/user_acc_test/sim_txt_files/bank_one_roll_then_quit.sim.txt")
    assert not errors, errors[0]


def test_bank_first_for_two_rounds():
    game = Game()
    errors = diff(game.play, path="tests/user_acc_test/sim_txt_files/bank_first_for_two_rounds.sim.txt")
    assert not errors, errors[0]